...
class TSP:
    ...
    def fitness_function(city_path):
        return city_path.get_path_distance()
    ...
    def solve(self):
        ...
        best_path = genetic_algo.evolve(50)
        print(genetic_algo.scores)
        print(f"Best score: {genetic_algo.best_score}")
        print(
            f"Best CityPath \ndistance: {round(best_path.get_path_distance())}")
        print([self.locations[i] for i in best_path.path])


if __name__ == "__main__":
    tsp = TSP(num_cities=10, population_size=100, num_generations=100)
